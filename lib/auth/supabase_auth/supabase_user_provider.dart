...
class SalesPortalSupabaseUser extends BaseAuthUser {
  SalesPortalSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> salesPortalSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SalesPortalSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
